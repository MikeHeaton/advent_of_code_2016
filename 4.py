# -*- coding: utf-8 -*-
"""
Created on Sun Dec  4 00:02:47 2016

@author: Mike
"""

stringinput='''aczupnetwp-mfyyj-opalcexpye-977[peyac]
...
byline = stringinput.split("\n")
counter = 0
reals = []
for line in byline:
    breakloc = line.find("[")
    counts = Counter(line[:breakloc])
    counts = [(x,counts[x]) for x in counts if x in "abcdefghijklmnopqrstuvwxyz"]
    counts.sort(key=lambda x:(x[1],-ord(x[0])), reverse=True)
    commonstr = "".join([x[0] for x in counts[:5]])
    if commonstr == line[breakloc+1:-1]:
        ident = int(line[breakloc-3:breakloc])
        counter += ident
        reals.append((ident, line[:breakloc-4]))
print(counter)

for ident, line in reals:
    newstr = ""
    for c in range(len(line)):
        newstr = newstr +  chr(97 + ((ord(line[c])-97+ident)%26))
    #if 'northpole' in newstr:
    print(ident, newstr)
